(function( $ ) {
    ...
    // ----- CHECKOUT FORM BEHAVIOUR -----
    $(".invoice-field input").each(function(){
        $(this).attr("disabled", "disabled");
    });

    $(function() {
        var $needInvoiceBtn = $('#billing_invoice_checkbox');
        $needInvoiceBtn.after('<label for="billing_invoice_checkbox">Need an invoice?</label>');
        $('#billing_euvat_checkbox').after('<label for="billing_euvat_checkbox">Active European VAT payer? <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="right" title="The European Union value added tax (or EU VAT) is a value added tax on goods and services within the European Union (EU)."></i></label>');
    ...
    $("#billing_euvat_checkbox").on("change", function(e) {
        if($(this).is(":checked")){
    ...
    $("#billing_invoice_checkbox").on("change", function() {
